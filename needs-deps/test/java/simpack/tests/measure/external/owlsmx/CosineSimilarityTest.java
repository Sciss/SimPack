...
public class CosineSimilarityTest extends TestCase {
	...
	public void testCalculationSimilarity() {
		CosineSimilarity cosine = new CosineSimilarity(profileDesc1,
				profileDesc2);
		assertNotNull(cosine);
		assertTrue(cosine.calculate());
		assertTrue(cosine.isCalculated());
		assertEquals(cosine.getSimilarity(), new Double(computeResultByHand()));
	}

	/**
	 * Cumbersomely reverse engineered from the OWLS-MX measures. Please note
	 * that for some reason IDFs get not computed correctly. Therefore, IDF is
	 * always 1.0.
	 * 
	...
	private double computeResultByHand() {
		...
		try {
			return (v1.getDotProduct(v2))
					/ (Math.sqrt(v1.getDotProduct(v1)) * Math.sqrt(v2
							.getDotProduct(v2)));
