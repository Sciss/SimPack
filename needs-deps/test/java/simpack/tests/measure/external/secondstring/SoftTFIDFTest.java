/*
 * $Id: SoftTFIDFTest.java 757 2008-04-17 17:42:53Z kiefer $
 *
 ...
 * @version $Id: SoftTFIDFTest.java 757 2008-04-17 17:42:53Z kiefer $
 */
public class SoftTFIDFTest extends TestCase {
	...
	public void testCalculateSimilarity() {
		softTFIDF = new SoftTFIDF(str1, str1, corpus,
				SimpleTokenizer.DEFAULT_TOKENIZER, new JaroWinkler(), 0.9);
		assertNotNull(softTFIDF);
		assertTrue(softTFIDF.calculate());
		assertTrue(softTFIDF.isCalculated());
		assertEquals(softTFIDF.getSimilarity(), new Double(1));
	}

	public void testCalculateSimilarity2() {
		softTFIDF = new SoftTFIDF(str1, str2, corpus,
				SimpleTokenizer.DEFAULT_TOKENIZER, new JaroWinkler(), 0.9);
		assertNotNull(softTFIDF);
		assertTrue(softTFIDF.calculate());
		assertTrue(softTFIDF.isCalculated());
		...
		assertEquals(softTFIDF.getSimilarity(), new Double(sim));
	}

	public void testCalculateSimilarityWithDifferentParameters() {
		softTFIDF = new SoftTFIDF(str1, str2, corpus,
				SimpleTokenizer.DEFAULT_TOKENIZER, new Jaro(), 0.9);
		assertNotNull(softTFIDF);
		assertTrue(softTFIDF.calculate());
		assertTrue(softTFIDF.isCalculated());
