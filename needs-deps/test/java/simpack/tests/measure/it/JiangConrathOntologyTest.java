/*
 * $Id: JiangConrathOntologyTest.java 757 2008-04-17 17:42:53Z kiefer $
 *
 ...
 * @version $Id: JiangConrathOntologyTest.java 757 2008-04-17 17:42:53Z kiefer $
 */
public class JiangConrathOntologyTest extends TestCase {
	...
	public void testJenaOntologyAccessor() {
		JiangConrath jc = null;
		try {
			jc = new JiangConrath(new JenaOntologyAccessor(SUMO_OntologyURI,
		...
		assertNotNull(jc);
		assertTrue(jc.calculate());
		assertTrue(jc.isCalculated());
		Double result = new Double(-2d * Math.log(19d / 121d)
				- (Math.log(1d / 121d) + Math.log(1d / 121d)));
		assertEquals(jc.getSimilarity(), result);
	}
	...
	public void testCalculateSimilarityMRCAIsRootOfOntology() {
		...
		assertTrue(jc.calculate());
		assertTrue(jc.isCalculated());
		Double result = new Double(-2d * Math.log(121d / 121d)
				- (Math.log(119d / 121d) + Math.log(3d / 121d)));
		assertEquals(jc.getSimilarity(), result);
	}
	...
	public void testCalculateSimilarityOneIsRootOfOntology() {
		...
		assertTrue(jc.isCalculated());
		Double result = new Double(-2d * Math.log(121d / 121d)
				- (Math.log(121d / 121d) + Math.log(119d / 121d)));
		assertEquals(jc.getSimilarity(), result);
	}
}