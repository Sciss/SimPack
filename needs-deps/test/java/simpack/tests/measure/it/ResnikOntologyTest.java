/*
 * $Id: ResnikOntologyTest.java 757 2008-04-17 17:42:53Z kiefer $
 *
 ...
 * @version $Id: ResnikOntologyTest.java 757 2008-04-17 17:42:53Z kiefer $
 */
public class ResnikOntologyTest extends TestCase {
	...
	public void testCalculateSimilarity() {
		Resnik resnik = null;
		try {
			resnik = new Resnik(new JenaOntologyAccessor(SUMO_OntologyURI,
		...
		Double result = new Double(-1d * Math.log(19d / 121d));
		assertEquals(resnik.getSimilarity(), result);
	}
	...
	public void testCalculateSimilaritySameClass() {
		...
		assertTrue(resnik.calculate());
		assertTrue(resnik.isCalculated());
		Double result = new Double(-1d * Math.log(3d / 121d));
		assertEquals(resnik.getSimilarity(), result);
	}
	...
	public void testCalculateSimilarityOneIsAncestorOfOther() {
		...
		assertTrue(resnik.calculate());
		assertTrue(resnik.isCalculated());
		Double result = new Double(-1d * Math.log(14d / 121d));
		assertEquals(resnik.getSimilarity(), result);
	}
	...
	public void testCalculateSimilarityMRCAIsRootOfOntology() {
		...
		assertTrue(resnik.calculate());
		assertTrue(resnik.isCalculated());
		Double result = new Double(-1d * Math.log(121d / 121d));
		assertEquals(resnik.getSimilarity(), result);
	}
}