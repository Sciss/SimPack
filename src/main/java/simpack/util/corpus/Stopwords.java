/*
 * $Id: Stopwords.java 757 2008-04-17 17:42:53Z kiefer $
 *
...
public class Stopwords {
	...
	static {
		if (m_Stopwords == null) {
			...
			// // added to avoid words likeI've, you've etc.
			// m_Stopwords.put("ve", dummy);
			...
			// Stopwords list from org.apache.lucene.analysis.StopAnalyzer
	...
	/**
	 * Returns true if the given util.string is a stop word.
	 */
	...
	public static String remove(String in) {
		String[] temp = null;
		temp = in.trim().split("\\s+");

		Vector<String> noStopWordsStr = new Vector<String>();

		for (int i = 0; i < temp.length; i++) {
			if (!isStopword(temp[i])) {
				noStopWordsStr.add(temp[i]);
			}
		}
