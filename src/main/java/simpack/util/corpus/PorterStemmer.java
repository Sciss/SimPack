/*
 * $Id: PorterStemmer.java 757 2008-04-17 17:42:53Z kiefer $
 *
 ...
 * in "An algorithm for suffix stripping", M.F. Porter (1980), _Program_, Vol.
 * 14, No. 3, pp. 130-137
 * 
 * @author John Keyes (keyeg@ils.unc.edu)
 * @version $Revision: 752 $ $Date: 2008-04-17 19:52:02 +0200 (Thu, 17 Apr 2008) $
 */
public class PorterStemmer {

	public String stem(String str) throws RuntimeException {
		...
		if (str.length() > 0) {
			...
			for (int i = 0; i < c.length; i++) {
				if (!Character.isLetter(c[i])) {
					// throw new RuntimeException("Invalid term: " + str);
					throw new RuntimeException();
				}
			}
		} else {
			// throw new RuntimeException("No term entered");
			throw new RuntimeException();
		}
		str = step1a(str);
		str = step1b(str);
	...
	// does util.string end with 's'?
	protected boolean endsWithS(String str) {
		return str.endsWith("s");
	} // end function

	// does util.string contain a vowel?
	...
	// does util.string end with a double consonent?
	...
	// returns a CVC simpack.measure for the util.string
